 ...
 */
    ...
    /* 인스턴스 초기화 블럭 */
    {
        ...
        * 하지만 static 초기화 블럭은 클래스로드 될 때 이미 기본값으로 초기화를 진행했다.
    ...
    static {
        /* 주의사항
        * static 초기화블럭에서는 non-static필드를 초기화 하지 못한다.
        * 정적 초기화 블럭은 클래스 로드시에 동작한다.
        * 따라서 정적 초기화 블럭의 동작 시점에는 인스턴스가 아무것도 존재할 수 없기 때문에
        * 존재하지 않은 인스턴스 변수에 초기화 하는 것은 시기상으로 불가능하다.
        * */
    ...
    public String getInformation(){
        return "product.getInformation() = Product [name = " + this.name + ", price = " + this.price + ", brand = " + Product.brand + "]";
    }
}
